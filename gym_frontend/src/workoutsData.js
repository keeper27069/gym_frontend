// workoutsData.js - Offline-first exercise catalog and gamification helpers
...
export function calculateLevel(streakWeeks = 0) {
  ...
  if (currentLvl >= 5) {
    ...
    const done = Math.max(0, streakWeeks - minW);
    progressPercent = Math.min(100, Math.floor((done / span) * 100));
    weeksLeft = Math.max(0, maxW - streakWeeks);
...
export function findWorkouts(goal = 'muscle_gain', gender = 'male', level = 1) {
  let matched = WORKOUT_SPLITS.filter((w) => {
    ...
    return goalMatch && genderMatch && w.difficulty_level <= level;
  });
  ...
  return [...matched].sort((a, b) => (a.day_number || 1) - (b.day_number || 1));
}
